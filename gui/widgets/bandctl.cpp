...
BandCtl::BandCtl(const int iBandNum, bool* bSemafor, const char* bundlepath, bool isStereo)
    ...
    m_Color = Gdk::RGBA(bandColorLUT[m_iBandNum]);
    ...
    m_img_ptr_loshelf =
        Gdk::Pixbuf::create_from_file(m_budlepath + "/" + std::string(LOSHEL_ICON_FILE));
    m_img_ptr_hishelf =
        Gdk::Pixbuf::create_from_file(m_budlepath + "/" + std::string(HISHEL_ICON_FILE));
    m_img_ptr_peak = Gdk::Pixbuf::create_from_file(m_budlepath + "/" + std::string(PEAK_ICON_FILE));
    m_img_ptr_notch =
        Gdk::Pixbuf::create_from_file(m_budlepath + "/" + std::string(NOTCH_ICON_FILE));

    m_FilterType = PEAK;
    loadTypeImg();
    set_size_request(46 + m_image_surface_ptr->get_width(),
                     (m_bIsStereoPlugin ? 80 : 65) + m_image_surface_ptr->get_height());
    ...
    itm_lpf = new Gtk::MenuItem("Low pass");
    itm_hpf = new Gtk::MenuItem("High pass");
    itm_loShel = new Gtk::MenuItem("Low Shelf");
    itm_hiShel = new Gtk::MenuItem("High Shelf");
    itm_peak = new Gtk::MenuItem("Peak");
    itm_notch = new Gtk::MenuItem("Notch");

	// set the menu item images
	itm_lpf->get_children().push_back(icon_lpf);
	itm_hpf->get_children().push_back(icon_hpf);
	itm_loShel->get_children().push_back(icon_loShel);
	itm_hiShel->get_children().push_back(icon_hiShel);
	itm_peak->get_children().push_back(icon_peak);
	itm_notch->get_children().push_back(icon_notch);
	...
	*/
    ...
    add_events(Gdk::BUTTON_PRESS_MASK | Gdk::BUTTON_RELEASE_MASK | Gdk::POINTER_MOTION_MASK |
               Gdk::SCROLL_MASK | Gdk::LEAVE_NOTIFY_MASK | Gdk::KEY_PRESS_MASK);
    signal_button_press_event().connect(sigc::mem_fun(*this, &BandCtl::on_button_press_event),
                                        true);
    signal_button_release_event().connect(sigc::mem_fun(*this, &BandCtl::on_button_release_event),
                                          true);
    signal_scroll_event().connect(sigc::mem_fun(*this, &BandCtl::on_scrollwheel_event), true);
    signal_motion_notify_event().connect(sigc::mem_fun(*this, &BandCtl::on_mouse_motion_event),
                                         true);
    signal_leave_notify_event().connect(sigc::mem_fun(*this, &BandCtl::on_mouse_leave_widget),
                                        true);
...
void BandCtl::loadTypeImg() {
    ...
    m_image_surface_ptr =
        Cairo::ImageSurface::create(format, img_ptr->get_width(), img_ptr->get_height());
...
// Data accesors
float BandCtl::getGain() { return m_GainBtn.value; }

float BandCtl::getFreq() { return m_FreqBtn.value; }

float BandCtl::getQ() { return m_QBtn.value; }

float BandCtl::getFilterType() { return (float)m_FilterType; }

bool BandCtl::getEnabled() { return m_bBandIsEnabled; }
...
BandCtl::signal_ctlBandChanged BandCtl::signal_changed() { return m_bandChangedSignal; }

BandCtl::signal_BandSelected BandCtl::signal_band_selected() { return m_bandSelectedSignal; }

BandCtl::signal_BandUnSelected BandCtl::signal_band_unselected() { return m_bandUnSelectedSignal; }

BandCtl::signal_MidSideChanged BandCtl::signal_mid_side_changed() { return m_midsideChangedSignal; }
...
bool BandCtl::on_button_press_event(GdkEventButton* event) {
    ...
    if (event->button == 1) {
        if (event->type == GDK_2BUTTON_PRESS) {
            ...
            m_GainBtn.text = (m_HpfLpf_slope == 0) & m_bBandIsEnabled &
                             (event->x > m_GainBtn.x0 && event->x < m_GainBtn.x1 &&
                              event->y > m_GainBtn.y0 && event->y < m_GainBtn.y1);
            m_FreqBtn.text =
                m_bBandIsEnabled & (event->x > m_FreqBtn.x0 && event->x < m_FreqBtn.x1 &&
                                    event->y > m_FreqBtn.y0 && event->y < m_FreqBtn.y1);
            m_QBtn.text = m_bBandIsEnabled & (event->x > m_QBtn.x0 && event->x < m_QBtn.x1 &&
                                              event->y > m_QBtn.y0 && event->y < m_QBtn.y1);
            if (m_GainBtn.text) {
                m_GainBtn.ss.str("");  // Clear stringstream
                m_GainBtn.ss << std::setprecision(2) << std::fixed << m_GainBtn.value;
                grab_focus();
                // keyPressEvent = signal_key_press_event().connect(sigc::mem_fun(*this,
                // &BandCtl::on_key_press_event)); //TODO
            ...
            m_EnableBtn.pressed = (event->x > m_EnableBtn.x0 && event->x < m_EnableBtn.x1 &&
                                   event->y > m_EnableBtn.y0 && event->y < m_EnableBtn.y1);

            // Simple click on the 1st button
            m_TypeBtn.pressed =
                m_bBandIsEnabled & (event->x > m_TypeBtn.x0 && event->x < m_TypeBtn.x1 &&
                                    event->y > m_TypeBtn.y0 && event->y < m_TypeBtn.y1);
            ...
            m_GainBtn.pressed =
                m_bBandIsEnabled & (event->x > m_GainBtn.x0 && event->x < m_GainBtn.x1 &&
                                    event->y > m_GainBtn.y0 && event->y < m_GainBtn.y1);
            m_FreqBtn.pressed =
                m_bBandIsEnabled & (event->x > m_FreqBtn.x0 && event->x < m_FreqBtn.x1 &&
                                    event->y > m_FreqBtn.y0 && event->y < m_FreqBtn.y1);
            m_QBtn.pressed = m_bBandIsEnabled & (event->x > m_QBtn.x0 && event->x < m_QBtn.x1 &&
                                                 event->y > m_QBtn.y0 && event->y < m_QBtn.y1);

            if (m_bIsStereoPlugin) {
                m_MidSideBtn.ML_pressed =
                    m_bBandIsEnabled & (event->x > m_MidSideBtn.Mx && event->x < m_MidSideBtn.Dx &&
                                        event->y > m_MidSideBtn.y0 && event->y < m_MidSideBtn.y1);
                m_MidSideBtn.Dual_pressed =
                    m_bBandIsEnabled & (event->x > m_MidSideBtn.Dx && event->x < m_MidSideBtn.Sx &&
                                        event->y > m_MidSideBtn.y0 && event->y < m_MidSideBtn.y1);
                m_MidSideBtn.SR_pressed =
                    m_bBandIsEnabled & (event->x > m_MidSideBtn.Sx && event->x < m_MidSideBtn.x1 &&
                                        event->y > m_MidSideBtn.y0 && event->y < m_MidSideBtn.y1);
...
bool BandCtl::on_button_release_event(GdkEventButton* event) {
    ...
    if (m_EnableBtn.pressed && (event->x > m_EnableBtn.x0 && event->x < m_EnableBtn.x1 &&
                                event->y > m_EnableBtn.y0 && event->y < m_EnableBtn.y1)) {
    ...
    if (m_bIsStereoPlugin && m_MidSideBtn.ML_pressed &&
        (event->x > m_MidSideBtn.Mx && event->x < m_MidSideBtn.Dx && event->y > m_MidSideBtn.y0 &&
         event->y < m_MidSideBtn.y1)) {
        m_MidSideBtn.State = ML;
        m_midsideChangedSignal.emit(m_iBandNum);
    }
    if (m_bIsStereoPlugin && m_MidSideBtn.Dual_pressed &&
        (event->x > m_MidSideBtn.Dx && event->x < m_MidSideBtn.Sx && event->y > m_MidSideBtn.y0 &&
         event->y < m_MidSideBtn.y1)) {
        m_MidSideBtn.State = DUAL;
        m_midsideChangedSignal.emit(m_iBandNum);
    }
    if (m_bIsStereoPlugin && m_MidSideBtn.SR_pressed &&
        (event->x > m_MidSideBtn.Sx && event->x < m_MidSideBtn.x1 && event->y > m_MidSideBtn.y0 &&
         event->y < m_MidSideBtn.y1)) {
...
bool BandCtl::on_mouse_motion_event(GdkEventMotion* event) {
    ...
    if (m_GainBtn.pressed) {
        ...
        m_FreqBtn.value +=
            (m_FreqBtn.value / 7.0f) * ((float)(event->x - m_iAntValueX) / ACCELERATION);
        ...
        m_EnableBtn.focus = (event->x > m_EnableBtn.x0 && event->x < m_EnableBtn.x1 &&
                             event->y > m_EnableBtn.y0 && event->y < m_EnableBtn.y1);
        m_TypeBtn.focus = (event->x > m_TypeBtn.x0 && event->x < m_TypeBtn.x1 &&
                           event->y > m_TypeBtn.y0 && event->y < m_TypeBtn.y1);
        m_GainBtn.focus = m_bBandIsEnabled & (event->x > m_GainBtn.x0 && event->x < m_GainBtn.x1 &&
                                              event->y > m_GainBtn.y0 && event->y < m_GainBtn.y1);
        m_FreqBtn.focus = m_bBandIsEnabled & (event->x > m_FreqBtn.x0 && event->x < m_FreqBtn.x1 &&
                                              event->y > m_FreqBtn.y0 && event->y < m_FreqBtn.y1);
        m_QBtn.focus = m_bBandIsEnabled & (event->x > m_QBtn.x0 && event->x < m_QBtn.x1 &&
                                           event->y > m_QBtn.y0 && event->y < m_QBtn.y1);

        if (m_bIsStereoPlugin) {
            m_MidSideBtn.Dual_focus =
                m_bBandIsEnabled & (event->x > m_MidSideBtn.Dx && event->x < m_MidSideBtn.Sx &&
                                    event->y > m_MidSideBtn.y0 && event->y < m_MidSideBtn.y1);
            m_MidSideBtn.ML_focus =
                m_bBandIsEnabled & (event->x > m_MidSideBtn.Mx && event->x < m_MidSideBtn.Dx &&
                                    event->y > m_MidSideBtn.y0 && event->y < m_MidSideBtn.y1);
            m_MidSideBtn.SR_focus =
                m_bBandIsEnabled & (event->x > m_MidSideBtn.Sx && event->x < m_MidSideBtn.x1 &&
                                    event->y > m_MidSideBtn.y0 && event->y < m_MidSideBtn.y1);
            redraw_MidSide_widget();
        }
    ...
    // Notify band over
    if (m_GainBtn.focus || m_FreqBtn.focus || m_QBtn.focus || m_TypeBtn.focus ||
        m_EnableBtn.focus || m_MidSideBtn.Dual_focus || m_MidSideBtn.ML_focus ||
        m_MidSideBtn.SR_focus) {
        m_bandSelectedSignal.emit(m_iBandNum);
    }
    redraw();
...
bool BandCtl::on_scrollwheel_event(GdkEventScroll* event) {
    ...
    if (m_bBandIsEnabled & (event->x > m_GainBtn.x0 && event->x < m_GainBtn.x1 &&
                            event->y > m_GainBtn.y0 && event->y < m_GainBtn.y1)) {
        if (m_HpfLpf_slope) {
            // HPF and LPF case
            increment *= 20.0;
    ...
    } else if (m_bBandIsEnabled & (event->x > m_FreqBtn.x0 && event->x < m_FreqBtn.x1 &&
                                   event->y > m_FreqBtn.y0 && event->y < m_FreqBtn.y1)) {
        ...
        m_FreqBtn.value = m_FreqBtn.value < FREQ_MIN ? FREQ_MIN : m_FreqBtn.value;
        m_bandChangedSignal.emit(m_iBandNum, FREQ_TYPE, m_FreqBtn.value);
    } else if (m_bBandIsEnabled & (event->x > m_QBtn.x0 && event->x < m_QBtn.x1 &&
                                   event->y > m_QBtn.y0 && event->y < m_QBtn.y1)) {
...
bool BandCtl::on_key_press_event(GdkEventKey* event) {
    std::cout << "BandCtl::on_key_press_event()"
              << " event = " << event->keyval << std::endl;  // TODO reomve
...
void BandCtl::setFilterTypeLPFHPFAcordSlope() {
    if (m_HpfLpf_slope < 40) {
        if (m_FilterType == LPF_ORDER_1 || m_FilterType == LPF_ORDER_2 ||
            m_FilterType == LPF_ORDER_3 || m_FilterType == LPF_ORDER_4) {
...
bool BandCtl::on_expose_event(GdkEventExpose* event) {
    ...
    if (window) {
        ...
        if (!m_midSide_surface_ptr && m_bIsStereoPlugin) {
            ...
            m_midSide_surface_ptr = Cairo::ImageSurface::create(
                Cairo::FORMAT_ARGB32, width, (height - 4.0 * OUTER_BORDER) / 4.0);
        }
        ...
        if (!m_bBtnInitialized) {
            ...
            m_GainBtn.y1 = m_EnableBtn.y1 + (height - 4.0 * OUTER_BORDER - m_EnableBtn.y1) /
                                                (m_bIsStereoPlugin ? 4.0 : 3.0);
            m_GainBtn.text = false;
            ...
            m_FreqBtn.y1 = m_EnableBtn.y1 + 2.0 * (height - 4.0 * OUTER_BORDER - m_EnableBtn.y1) /
                                                (m_bIsStereoPlugin ? 4.0 : 3.0);
            m_FreqBtn.text = false;
            ...
            m_QBtn.y1 = m_EnableBtn.y1 + 3.0 * (height - 4.0 * OUTER_BORDER - m_EnableBtn.y1) /
                                             (m_bIsStereoPlugin ? 4.0 : 3.0);
            m_QBtn.text = false;
        ...
        cr->arc(width - OUTER_BORDER - radius, OUTER_BORDER + radius, radius, -90 * degrees,
                0 * degrees);
        ...
        cr->set_line_width(1);
        if (m_bBandIsEnabled) {
            cr->set_source_rgba(m_Color.get_red_p(), m_Color.get_green_p(), m_Color.get_blue_p(),
                                0.5);
        } else {
        ...
        Cairo::RefPtr<Cairo::LinearGradient> bkg_gradient_ptr = Cairo::LinearGradient::create(
            width / 2, OUTER_BORDER, width / 2, height - OUTER_BORDER);
        if (m_bBandIsEnabled) {
            bkg_gradient_ptr->add_color_stop_rgba(0.0, m_Color.get_red_p(), m_Color.get_green_p(),
                                                  m_Color.get_blue_p(), 0.6);
            bkg_gradient_ptr->add_color_stop_rgba(1.0, m_Color.get_red_p(), m_Color.get_green_p(),
                                                  m_Color.get_blue_p(), 0.3);
        } else {
            bkg_gradient_ptr->add_color_stop_rgba(0.0, 0.8, 0.8, 0.8, 0.4);
            bkg_gradient_ptr->add_color_stop_rgba(1.0, 0.8, 0.8, 0.8, 0.2);
        }
        bkg_gradient_ptr->add_color_stop_rgba(0.3, m_Color.get_red_p(), m_Color.get_green_p(),
                                              m_Color.get_blue_p(), 0.1);
        bkg_gradient_ptr->add_color_stop_rgba(0.5, m_Color.get_red_p(), m_Color.get_green_p(),
                                              m_Color.get_blue_p(), 0.0);
        bkg_gradient_ptr->add_color_stop_rgba(0.9, m_Color.get_red_p(), m_Color.get_green_p(),
                                              m_Color.get_blue_p(), 0.05);
        ...
        cr->rectangle(m_TypeBtn.x0 - 0.5, m_TypeBtn.y0 - 0.5, m_image_surface_ptr->get_width(),
                      m_image_surface_ptr->get_height());
        cr->clip();
        cr->paint();
        cr->restore();
        ...
        if (m_bBandIsEnabled) {
            cr->save();
            cr->begin_new_sub_path();
            cr->arc(m_TypeBtn.x1 - radius - 1 - 0.5, m_TypeBtn.y0 + radius + 0.5, radius,
                    -90 * degrees, 0 * degrees);
            cr->arc(m_TypeBtn.x1 - radius - 1 - 0.5, m_TypeBtn.y1 - radius - 0.5, radius,
                    0 * degrees, 90 * degrees);
            cr->arc(m_TypeBtn.x0 + radius + 0.5, m_TypeBtn.y1 - radius - 0.5, radius, 90 * degrees,
                    180 * degrees);
            cr->arc(m_TypeBtn.x0 + radius + 0.5, m_TypeBtn.y0 + radius + 0.5, radius, 180 * degrees,
                    270 * degrees);
            cr->close_path();
...
void BandCtl::drawBandButton(BandCtl::Button* btn, Cairo::RefPtr<Cairo::Context> cr) {
    ...
    if (btn->focus) {
        Cairo::RefPtr<Cairo::LinearGradient> bkg_gradient_ptr =
            Cairo::LinearGradient::create(btn->x0 + 0.5 * (btn->x1 - btn->x0), btn->y0,
                                          btn->x0 + 0.5 * (btn->x1 - btn->x0), btn->y1);
        bkg_gradient_ptr->add_color_stop_rgba(0.1, m_Color.get_red_p() + 0.2,
                                              m_Color.get_green_p() + 0.2,
                                              m_Color.get_blue_p() + 0.2, 0.0);
        bkg_gradient_ptr->add_color_stop_rgba(0.45, m_Color.get_red_p() + 0.2,
                                              m_Color.get_green_p() + 0.2,
                                              m_Color.get_blue_p() + 0.2, 0.3);
        bkg_gradient_ptr->add_color_stop_rgba(0.55, m_Color.get_red_p() + 0.2,
                                              m_Color.get_green_p() + 0.2,
                                              m_Color.get_blue_p() + 0.2, 0.3);
        bkg_gradient_ptr->add_color_stop_rgba(0.9, m_Color.get_red_p() + 0.2,
                                              m_Color.get_green_p() + 0.2,
                                              m_Color.get_blue_p() + 0.2, 0.0);
        cr->save();
        cr->set_source(bkg_gradient_ptr);
    ...
    if (btn->units != "dB/dec") {
        ...
        ss << std::setprecision(precision) << std::fixed
           << (btn->value < 1e3 ? btn->value : (0.001 * (btn->value)))
           << (btn->value < 1e3 ? " " : " k") << (btn->units);
    } else {
...
void BandCtl::redraw_MidSide_widget() {
    if (m_midSide_surface_ptr && m_bIsStereoPlugin) {
        ...
        cr->save();
        Cairo::RefPtr<Cairo::LinearGradient> bkg_gradient_ptr =
            Cairo::LinearGradient::create(0, 0, 0, m_midSide_surface_ptr->get_height());
        switch (m_MidSideBtn.State) {
            ...
            case ML:
                cr->begin_new_sub_path();
                cr->arc(m_MidSideBtn.x0 + btnH / 2.0, btnH / 2.0 + 0.5, btnH / 2.0, M_PI / 2.0,
                        -M_PI / 2.0);
            ...
            case SR:
                cr->begin_new_sub_path();
                cr->arc(m_MidSideBtn.x1 - btnH / 2.0, btnH / 2.0 + 0.5, btnH / 2.0, -M_PI / 2.0,
                        M_PI / 2.0);
        ...
        bkg_gradient_ptr->add_color_stop_rgba(0.7, m_Color.get_red_p(), m_Color.get_green_p(),
                                              m_Color.get_blue_p(), 0.6);
        ...
        cr->arc(Xcenter - btnH / 4.0 + (btnH / 10.0), (btnH / 2.0) + 0.5, btnH / 3.5, 0.0,
                2.0 * M_PI);
        cr->stroke();
...
BandCtl::MSState BandCtl::getStereoState() { return m_MidSideBtn.State; }
