...
VUWidget::VUWidget(int iChannels, float fMin, float fMax, std::string title, bool IsGainReduction,
                   bool DrawThreshold)
    ...
    if (m_bDrawThreshold) {
        ...
        signal_button_press_event().connect(sigc::mem_fun(*this, &VUWidget::on_button_press_event),
                                            true);
        signal_button_release_event().connect(
            sigc::mem_fun(*this, &VUWidget::on_button_release_event), true);
        signal_scroll_event().connect(sigc::mem_fun(*this, &VUWidget::on_scrollwheel_event), true);
        signal_motion_notify_event().connect(sigc::mem_fun(*this, &VUWidget::on_mouse_motion_event),
                                             true);
        signal_leave_notify_event().connect(sigc::mem_fun(*this, &VUWidget::on_mouse_leave_widget),
                                            true);
    }

    Glib::signal_timeout().connect(sigc::mem_fun(*this, &VUWidget::on_timeout_redraw),
...
void VUWidget::setValue(int iChannel, float fValue) {
    if (fValue > 0) {
        if (m_iBuffCnt[iChannel] > 0) {
            m_fValues[iChannel] =
                ((((double)m_iBuffCnt[iChannel]) * m_fValues[iChannel]) + 20.0 * log10(fValue)) /
                ((double)(m_iBuffCnt[iChannel] + 1));
        } else {
...
void VUWidget::clearPeak(int iChannel) { m_fPeaks[iChannel] = 0.0; }
...
bool VUWidget::on_expose_event(GdkEventExpose* event) {
    ...
    if (window) {
        ...
        if (!(m_background_surface_ptr || m_foreground_surface_ptr || m_fader_surface_ptr)) {
            m_background_surface_ptr =
                Cairo::ImageSurface::create(Cairo::FORMAT_ARGB32, width, height);
            redraw_background();
            m_foreground_surface_ptr =
                Cairo::ImageSurface::create(Cairo::FORMAT_ARGB32, width, height);
            redraw_foreground();
...
void VUWidget::set_value_th(double value) {
    ...
    m_ThFaderValue = m_ThFaderValue > m_fMax - 2.0
                         ? m_fMax - 2.0
                         : m_ThFaderValue;  // Limit threshols 2dB less than VU
    m_redraw_fader = true;
}

double VUWidget::get_value_th() { return m_ThFaderValue; }
...
bool VUWidget::on_mouse_motion_event(GdkEventMotion* event) {
    if (bMotionIsConnected) {
        ...
        m_FaderFocus = event->y > m_iThFaderPositon - MICROFADER_WIDTH / 2 &&
                       event->y < m_iThFaderPositon + MICROFADER_WIDTH / 2 &&
...
VUWidget::signal_FaderChanged VUWidget::signal_changed() { return m_FaderChangedSignal; }
...
void VUWidget::redraw_background() {
    if (m_background_surface_ptr) {
        ...
        cr->move_to(MARGIN + TEXT_OFFSET - 3,
                    TOP_OFFSET / 2);  // 4 is to get the text centered in VU
        pangoLayout->set_text(m_Title.c_str());
        pangoLayout->set_width(Pango::SCALE *
                               (CHANNEL_WIDTH * m_iChannels + (m_iChannels - 1) * MARGIN));
        ...
        for (int i = 0; i < m_iChannels; i++) {
            ...
            cr->arc(
                MARGIN + TEXT_OFFSET + CHANNEL_WIDTH + i * (MARGIN + CHANNEL_WIDTH + 0.5) - radius,
                MARGIN + TOP_OFFSET - 4 + radius, radius, -90 * degrees, 0 * degrees);
            cr->arc(
                MARGIN + TEXT_OFFSET + CHANNEL_WIDTH + i * (MARGIN + CHANNEL_WIDTH + 0.5) - radius,
                height - 1 - MARGIN - radius, radius, 0 * degrees, 90 * degrees);
            cr->arc(MARGIN + TEXT_OFFSET + i * (MARGIN + CHANNEL_WIDTH + 0.5) + radius,
                    height - 1 - MARGIN - radius, radius, 90 * degrees, 180 * degrees);
            cr->arc(MARGIN + TEXT_OFFSET + i * (MARGIN + CHANNEL_WIDTH + 0.5) + radius,
                    MARGIN + TOP_OFFSET - 4 + radius, radius, 180 * degrees, 270 * degrees);
            cr->close_path();
...
void VUWidget::redraw_foreground() {
    if (m_foreground_surface_ptr) {
        ...
        for (float fdb = m_fMin; fdb <= m_fMax; fdb = fdb + m_textdBseparation) {
            ...
            cr->line_to(MARGIN + TEXT_OFFSET + CHANNEL_WIDTH +
                            (m_iChannels - 1) * (CHANNEL_WIDTH + MARGIN) + 2,
                        round(dB2Pixels(fdb)) + 0.5);
            cr->stroke();
        }
        cr->restore();
    }
...
void VUWidget::redraw_faderwidget() {
    if (m_fader_surface_ptr) {
        ...
        Cairo::RefPtr<Cairo::RadialGradient> bkg_gradient_rad_ptr =
            Cairo::RadialGradient::create(0, 0, 0, 0, 0, 1);
        ...
        double degrees = M_PI / 180.0;
        cr->begin_new_sub_path();
        cr->arc(width - 2 - MICROFADER_WIDTH / 4, m_iThFaderPositon + 0.5, MICROFADER_WIDTH / 4,
                -90 * degrees, 90 * degrees);
        cr->line_to(width - 2 - MICROFADER_WIDTH / 2,
                    m_iThFaderPositon + MICROFADER_WIDTH / 4 + 0.5);
        ...
        Cairo::RefPtr<Cairo::LinearGradient> bkg_gradient_ptr = Cairo::LinearGradient::create(
            width - 2 - MICROFADER_WIDTH / 2, m_iThFaderPositon - MICROFADER_WIDTH / 4,
            width - 2 - MICROFADER_WIDTH / 2, m_iThFaderPositon + MICROFADER_WIDTH / 4);
        ...
        // Draw focus glow
        if (m_FaderFocus) {
            Cairo::RefPtr<Cairo::RadialGradient> glow_gradient_rad_ptr =
                Cairo::RadialGradient::create(width - MICROFADER_WIDTH / 2, m_iThFaderPositon,
                                              MICROFADER_WIDTH / 2, width - MICROFADER_WIDTH / 2,
                                              m_iThFaderPositon, MICROFADER_WIDTH);
    ...
    if (m_vu_surface_ptr) {
        ...
        for (int i = 0; i < m_iChannels; i++) {
            ...
            bkg_gradient_ptr = Cairo::LinearGradient::create(
                MARGIN + TEXT_OFFSET + CHANNEL_WIDTH / 2.0 + i * (MARGIN + CHANNEL_WIDTH + 0.5),
                dB2Pixels(m_fMin),
                MARGIN + TEXT_OFFSET + CHANNEL_WIDTH / 2.0 + i * (MARGIN + CHANNEL_WIDTH + 0.5),
                dB2Pixels(m_fMax));
            if (m_bIsGainReduction) {
