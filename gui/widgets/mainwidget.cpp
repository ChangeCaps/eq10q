...
MainWidget::MainWidget() { set_border_width(BORDER); }
...
void MainWidget::on_realize() {
    ...
    m_WinBgColor.set_rgb(GDK_COLOR_MACRO(BACKGROUND_R), GDK_COLOR_MACRO(BACKGROUND_G),
                         GDK_COLOR_MACRO(BACKGROUND_B));
...
void MainWidget::on_realize() {
	Gtk::EventBox::on_realize();
	Glib::RefPtr<Gtk::StyleContext> m_style = this->get_style_context();
	this->m_bg_orgi = m_style->get_background_color(Gtk::STATE_FLAG_NORMAL);
	
	// Set Main widget Background
	Gdk::RGBA m_WinBgColor;
	m_WinBgColor.set_rgba(BACKGROUND_R, BACKGROUND_G, BACKGROUND_B);
	this->override_background_color(m_WinBgColor);
...
bool MainWidget::send_expose(GdkEvent* event) {
	bool ret = Gtk::EventBox::send_expose(event);  // Call parent redraw()
    ...
    if (window) {
        ...
        cr->line_to(0, 0);
        cr->close_path();
        cr->set_source_rgb(m_bg_orgi.get_red(), m_bg_orgi.get_green(), m_bg_orgi.get_blue());
        cr->fill();
        cr->restore();
